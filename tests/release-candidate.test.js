...
(async () => {
  ...
  assert.equal(version, "v1.6.0", "Service Worker 缓存版本错误");
  ...
  assert.deepEqual([...stores.get(cacheName).keys()].sort(), [...shell].sort(), "v1.6.0 应用壳缓存不完整");
  ...
  assert.equal(stores.has(cacheName), true, "v1.6.0 缓存被误删");
  ...
  console.log(`v1.6.0 release candidate: ok${process.argv.includes("--release") ? " (Git ready)" : ""}`);
