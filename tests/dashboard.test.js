...
const requiredSections = ["meta", "travelers", "route", "flights", "hotels", "itinerary", "transportPlan", "foodRecommendations", "travelTips", "departureChecklist", "finalDepartureChecklist", "departureCountdown", "riskAlerts", "coupleMoments", "tasks", "packing", "documents", "budget", "emergency", "gallery", "alerts", "changeLog"];
requiredSections.forEach((key) => assert.ok(data[key], `缺少数据区块: ${key}`));
assert.equal(data.meta.version, "1.3.0", "版本未更新");
assert.equal(data.meta.versionName, "Shared Checklist Sync", "版本名称未更新");
...
assert.match(serviceWorker, /v1\.3\.0/, "Service Worker缓存版本未更新");
