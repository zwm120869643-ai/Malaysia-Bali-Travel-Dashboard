...
(async () => {
  ...
  assert.match(app, /id="flight-watcher-form"/, "Command Center 缺少航班号查询入口");
  assert.match(app, /flightWatch\?\.statusLabel \|\| live\.flight\?\.actualStatus/, "网络失败时未回退手动状态");
  ...
  assert.match(edge, /claims\.role === "authenticated"/, "Edge Function 未限制为已登录用户");
