...
- 酒店与航班：Registry 从现有公开数据生成，避免维护两份记录。
- Travel Inbox：当前只定义 PDF/图片入口元数据，文件内容必须进入私人存储层，不上传 GitHub 或公开 Supabase。
- Offline Pack：缓存公开航班、酒店、紧急信息、行程和清单摘要；不包含证件、订单号或支付资料。
- Readiness Board：从现有 Documents、Hotels、Flights、Network 和 Activities 状态实时计算，不维护第二份进度。
